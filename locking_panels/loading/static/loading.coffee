...
checkPromiseNum = ->
  if promise_num < 0
    throw new RangeError(
      'The number of promises registered in this module
      cannot be negative!')
...
resolvePromise = ->
  ...
  unless promise_num
    resolveLoadPromise()
    #@load_promise = null # load_promise stays defined until
                          # next showLoading invocation
    resolveLoadPromise = null
...
@showLoading =
  (message='Loading ...', promise=null, min_time=5000) =>
    if promise_num == 0
      switchToPanel('loading-panel')
      addPromise(
        new Promise(
          (resolve, reject) -> setTimeout resolve, min_time
        )
      )
      @load_promise = new Promise(
        (resolve, reject) -> resolveLoadPromise = resolve
      )
